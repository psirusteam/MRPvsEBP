...
poststrat_df <- readRDS("COL/2019/2.Pobreza/Data/poststrat_df.RDS")
...
poststrat_df %>% summarise(
  Nacional_lmer = sum(n * yk_lmer) / sum(n/gk),
...
bynames <-
  grep(
    pattern =  "^(X|F|.gruops|n|lp|pobreza|ingreso|tasa_desocupacion|epred_mat|gk|yk)",
...
plot_uni <- plot_uni$depto$Plot$plot1  / 
  ( plot_uni$sexo$Plot$plot1 + plot_uni$anoest$Plot$plot1 +
      ...
      plot_uni$area$Plot$plot1+ plot_uni$discapacitado$Plot$plot1)

ggsave(plot = plot_uni, 
       filename = "COL/2019/2.Pobreza/Output/plot_uni2.jpeg",scale = 5)
###########################################
###########################################
...
#     ggsave(
#       paste0(
#         "COL/2019/2.Pobreza/Output/plot2_",
